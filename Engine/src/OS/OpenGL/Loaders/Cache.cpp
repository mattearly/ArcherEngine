...
int Cache::try_load_from_cache(std::vector<MeshInfo>& out_meshes, Skeleton& skel, const std::string& path_key) {
  for (auto& ref_model_info : all_loaded_animated_scenes_) {
    if (ref_model_info.path == "")
      continue;
...
void Cache::remove_anim_scene_at_path(const std::string& path_to_remove) {
  // step1: decrement loaded count
  for (auto& ref_model_info : all_loaded_animated_scenes_) {
    if (ref_model_info.path == "")
      continue;
  ...
  // c) C++17 (and maybe some older) method to remove from foward_list
  auto before = all_loaded_animated_scenes_.before_begin();
  for (auto it = all_loaded_animated_scenes_.begin(); it != all_loaded_animated_scenes_.end(); ) {
    if (it->ref_count < 1) {
      it = all_loaded_animated_scenes_.erase_after(before);
    } else {
      before = it;
      ++it;
    }
...
void Cache::remove_textures(const TextureMapType& to_remove) {
  // step1: decrement loaded count
  for (auto& remove_tex : to_remove) {
    for (auto& old_tex : all_loaded_2d_textures_) {
      if (old_tex.path == "")
        continue;
      if (remove_tex.path.data() == old_tex.path.data()) {
        if (old_tex.ref_count > 0) {
          old_tex.ref_count--;
        }
      }
    }
...
void Cache::add(const AnimSceneInfo& asi) {
  all_loaded_animated_scenes_.push_front(asi);
}
